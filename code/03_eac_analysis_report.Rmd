...
# change coffee NA values to 0
eac %<>% 
  mutate(
    p_cafe = if_else(is.na(p_cafe),
                          0,
                          p_cafe),
    r_cafe = if_else(is.na(r_cafe),
                          0,
                          r_cafe),
    
    coffee_avg = if_else(is.na(coffee_avg),
                          0,
                          coffee_avg)
...
summarized_vars %<>% rename(Variable = name_var, Mean = mean, Std_Dev = sd,
...
# html output
kable(summarized_vars, digits = 2)

kable(summarized_vars, format = "latex", digits = 2,
      caption = "Descriptive Statistics for Variables", booktabs = T) %>%
  kable_styling(latex_options = c("striped", "scale_down")) %>% 
  kable_as_image(filename = "variable_summary",
               file_format = "pdf",
               density = 300)
```
...
ggplot(eac) + geom_bar(aes(x = illegal_crops)) +
  labs(title = "Distribution of Coca Presence Dummy (2013)",
       x = "Coca Presence Dummy ('illegal_crops')")
ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats/coca_dummy_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)

eac %>% select(illegal_crops) %>% count()

# vio_13 (coca presence dummy)
ggplot(eac) + geom_freqpoly(aes(x = vio_13), binwidth = 0.05) +
  labs(title = "Distribution of Coca Presence Avg. (1998-2014)",
       x = "Coca Presence Avg. ('vio_13')")
ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats/coca_avg_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
ggplot(data = eac) + geom_bar(mapping = aes(x = muni_locd)) +
  labs(title = "How many municipalities receive agricultural aid?",
       subtitle = "1 = receives aid",
       x = "Targeted with Ag. Aid ('muni_locd')")
ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats/aid_distribution.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
ggplot(eac) +
  ...
  xlim(-500, 500) +
  labs(title = "Distribution of MODIS Cropland Cover Trend*",
       caption = "*Zoomed in on x-axis for better visualization. Leaves out
       around ~10 values.",
       x = "MODIS Cropland Cover Trend ('trendcrop')")

ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats/trendcrop_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
```
...
ggplot(eac) +
  ...
  xlim(-2000, 4000) +
  labs(title = "Distribution of MODIS Cropland/Natural Vegetation Cover Trend*",
       caption = "*Zoomed in on x-axis for better visualization. Leaves out
       around ~10 values.",
       x = "MODIS Crop/Natural Veg. Cover Trend ('trendcropveg')")

ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats/trendcropveg_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
ggplot(eac) + geom_histogram(aes(x = vio_31_mean), binwidth = .1) +
  labs(title = "Population's Contact with Government/State Military",
       subtitle = "the proportion of years with contact",
       x = "Population Exposure to State ('vio_31_mean')")

ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats/popexpo_state_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
ggplot(eac) + geom_histogram(aes(x = vio_32_mean), binwidth = .1) +
  labs(title = "Population's Contact with Guerilla Forces",
       subtitle = "the proportion of years with contact",
       x = "Population Exposure to Guerillas ('vio_32_mean')")

ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats/popexpo_guer_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
ggplot(eac) + geom_histogram(aes(x = vio_34_mean), binwidth = .1) +
  labs(title = "State vs. Guerilla Disputes in a Municipality (1998-2014)",
       subtitle = "proportion of years with fighting",
       x = "State vs. Guerilla Dispute ('vio_34_mean')")

ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats",
                 "state-guerilla-dispute_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
ggplot(eac) + geom_bar(aes(x = vio_46)) +
  labs(title = "Conflict Incidence in a Municipality (2002-2013)",
       x = "Conflict Incidence Index* ('vio_46')",
       caption = "*Dept of National Planning, Colombia")
ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats",
                 "incidence_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
ggplot(cropveg_out, aes(muni_locd, trendcropveg, group = muni_locd)) +
  geom_boxplot() +
  labs(title = "Aid and Cropland/Natural Veg. Cover (Trend)*",
       subtitle = "1 = municipality receives aid",
       x = "Receives Aid? ('muni_locd')",
       y = "Trend in Crop/Natural Veg Cover ('trendcropveg')",
       caption = "*Not including the outlier Uribia (La Guajira)")

ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats",
                 "cropveg_aid_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 7,
       height = 7, units = "in", dpi = 300)
...
crop_out %>% filter(trendcrop < 2500) %>%
  ...
  geom_boxplot() +
  labs(title = "Aid and Cropland Cover (Trend)*",
       subtitle = "1 = municipality receives aid",
       x = "Receives Aid? ('muni_locd')",
       y = "Trend in Crop Cover ('trendcrop')",
       caption = "*Not showing 3 outliers (e.g. Uribia (La Guajira))"
       )

ggsave(file.path(dir.figures.tables,
                 "appendix/descriptive_stats",
                 "crop_aid_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 8,
...
## Aid, Coffee, and Coca
```{r coffee-model, warning=FALSE}
## models ----------------------------------------------------------------------

## VARS TO INCLUDE IN MODEL
base_vars <- ~muni_locd + SPI12m_sd + geo_3 + upstream + altitude +
                vio_13 + p_cafe + vio_31_mean + vio_32_mean
...
```{r coffee-output, warning=FALSE}
## output regression tables ----------------------------------------------------

## FILENAME STRING
filename <- "cropveg_coffee_prod_table.doc"

filename2 <- "crop_coffee_prod_table.doc"
...
coefnames <- list("(Intercept)" = NA, "muni_locd" = "Aid given",
                  ...
                  "vio_32_mean" = "Exposure to Guerillas",
                  "vio_13" = "Coca Presence Avg",
                  "muni_locd:p_cafe" = "Aid * Coffee Production",
                  "p_cafe" = "Coffee Production (Ha)")

## NOTE FOR BOTTOM OF TABLE
# %stars puts p-value definitions in Note
note <- "%stars. OLS regressions with MODIS cropveg trend as the dependent
variable. Table shows robust SEs."
...
ggsave("cropveg_coffee_prod.pdf", plot = last_plot(),
       device = "pdf", width = 7, height = 7, units = "in", dpi = 300)

## CROP PLOTS
interplot(m = linear.model2[[2]], var1 = "muni_locd", var2 = "p_cafe",
          hist = TRUE) +
  ylab("Aid's Effect on Crop Cover (in Ha)") +
  xlab("Avg. Coffee Production") +
  geom_hline(yintercept = 0, color="grey35", size = .3) +
  ggtitle("Estimated Coefficient of Aid by Coffee Production (1993-2015)")
ggsave("crop_coffee_prod.pdf", plot = last_plot(),
       device = "pdf", width = 7, height = 7, units = "in", dpi = 300)
...
```{r, warning=FALSE}
xtabs(~eac$illegal_crops + eac$coffee_2014)
```
There's only a 50% overlap in coffee (2014)  and coca (2013) municipalities. I would expect that if you're able to grow coca you're going to stick with growing that instead of coffee if you can. It probably brings in more money, requires less work, etc. In areas that are mixed with both it could be a function of the municipality's size and maybe a sign of some crackdown on coca growth? It would also be interesting to see this in FARC zones/non-FARC zones.

```{r coff-coca-combo-df, warning=FALSE, include=FALSE}
## CREATE DF TO SEE CROP/CROPVEG AND COFFEE/COCA/AID COMBOS
# RECEIVE AID
coffee_coca_aid <- eac %>%
  select(codmuni, municipality, department,
         coffee_2014, illegal_crops, muni_locd,
         trendcropveg, trendcrop
  ) %>%
  filter(muni_locd == 1
  ) %>% 
  mutate(
    coffee_coca = case_when(
      coffee_2014 == 1 & illegal_crops == 1 ~ "Coff-Y-Coca-Y",
      coffee_2014 == 1 & illegal_crops == 0 ~ "Coff-Y-Coca-N",
      coffee_2014 == 0 & illegal_crops == 1 ~ "Coff-N-Coca-Y",
      coffee_2014 == 0 & illegal_crops == 0 ~ "Coff-N-Coca-N",
      TRUE ~ NA_character_
      )
  )
coffee_coca_aid$coffee_coca <- as.factor(coffee_coca_aid$coffee_coca)

# DON'T RECEIVE AID
coffee_coca_no_aid <- eac %>%
  select(codmuni, municipality, department,
         coffee_2014, illegal_crops, muni_locd,
         trendcropveg, trendcrop
  ) %>%
  filter(muni_locd == 0 & codmuni != 44847
  ) %>% 
  mutate(
    coffee_coca = case_when(
      coffee_2014 == 1 & illegal_crops == 1 ~ "Coff-Y-Coca-Y",
...
coffee_coca_no_aid$coffee_coca <- as.factor(coffee_coca_no_aid$coffee_coca)
```


```{r, warning=FALSE}
## CROPVEG TREND ACROSS DIFFERENT COFFEE-COCA COMBOS

# strings for titled, labels, legends
legend_name <- "Coffee-Coca Presence?"
legend_breaks <- c("Coff-N-Coca-N",
                   "Coff-N-Coca-Y",
                   "Coff-Y-Coca-N",
                   "Coff-Y-Coca-Y")
legend_labels <- c("Both No",
                   "Coffee (No)/Coca (Yes)",
                   "Coffee (Yes)/Coca (No)",
                   "Both Yes")
xaxis_var <- "Coffee & Coca Combo"
yaxis_var_cropveg <- "Crop/Natural Veg Cover"
yaxis_var_crop <- "Crop Cover"
coff_coca_aid_title_cropveg <- "Crop/Natural Veg. Cover in Municipalities Receiving Aid*"
coff_coca_aid_title_crop <- "Crop Cover in Municipalities Receiving Aid*"
coff_coca_noaid_title_cropveg <- "Crop/Natural Veg. Cover in Municipalities Not Receiving Aid*"
coff_coca_noaid_title_crop <- "Crop Cover in Municipalities Not Receiving Aid*"

# function returning the n of each group to display on boxplot
give.n <- function(x){
  return(c(y = median(x)*1.1, label = length(x))) 
  # experiment with the multiplier to find the perfect position
}

# receiving aid + cropveg
ggplot(coffee_coca_aid, aes(x = coffee_coca, y = trendcropveg,
                            fill = coffee_coca)) +
  ...
  labs(title = coff_coca_aid_title_cropveg,
       x = xaxis_var,
       y = yaxis_var_cropveg,
       caption = "*n = 119 (w/ 'NA' = 1 muni)"
  )

ggsave(file.path(dir.figures.tables,
                 "cropveg_coffee-coca_aid_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 9,
       height = 7, units = "in", dpi = 300)


# receiving aid + crop
ggplot(coffee_coca_aid, aes(x = coffee_coca, y = trendcrop,
                            fill = coffee_coca)) +
  geom_boxplot(notch = FALSE
  ) +
  stat_summary(fun.data = give.n, geom = "text", fun.y = median
  ...
  labs(title = coff_coca_aid_title_crop, # make sure this is NOT 'cropveg'
       x = xaxis_var,
       y = yaxis_var_crop, # make sure this is NOT 'y_axis_var_cropveg'
       caption = "*n = 119 (w/ 'NA' = 1 muni)"
  )

ggsave(file.path(dir.figures.tables,
                 "crop_coffee-coca_aid_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 9,
       height = 7, units = "in", dpi = 300)



# NOT receiving aid + cropveg
ggplot(coffee_coca_no_aid,
       aes(x = coffee_coca, y = trendcropveg, fill = coffee_coca)
  ) +
  geom_boxplot(notch = FALSE
  ) +
  stat_summary(fun.data = give.n, geom = "text", fun.y = median
  ...
  labs(title = coff_coca_noaid_title_cropveg,
       x = xaxis_var,
       y = yaxis_var_cropveg,
       caption = "*n = 1002 (w/ 'NA' = 25 munis)"
  )

ggsave(file.path(dir.figures.tables,
                 "cropveg_coffee-coca_no-aid_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 9,
       height = 7, units = "in", dpi = 300)



# NOT receiving aid + crop
ggplot(coffee_coca_no_aid,
       aes(x = coffee_coca, y = trendcrop, fill = coffee_coca)
  ) +
  ylim(-2000, 500
  ) +
  geom_boxplot(notch = FALSE
  ...
  labs(title = coff_coca_noaid_title_crop, # make sure this is NOT 'cropveg'
       x = xaxis_var,
       y = yaxis_var_crop, # make sure this is NOT 'y_axis_var_cropveg'
       caption = "*n = 1002 (w/ 'NA' = 5 munis)"
  )

ggsave(file.path(dir.figures.tables,
                 "crop_coffee-coca_no-aid_distrib.pdf"),
       plot = last_plot(), device = "pdf", width = 9,
       height = 7, units = "in", dpi = 300)


## N per Coffee/Coca Grouping
# coffee_coca_aid %>% count(coffee_coca)
# coffee_coca_no_aid %>% count(coffee_coca)
...
```{r aid-coca-dummy, echo = FALSE, warning=FALSE}
# 1) base model: cropshare = aid + coca
# 2) base model + aid*coca
# 3) base model + pol vars (_match_none dropped out!)
# 4) base model + aid*coca + pol vars
